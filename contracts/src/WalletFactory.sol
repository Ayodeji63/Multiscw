...
contract WalletFactory {
    Wallet public immutable walletImplementaion;

    constructor(IEntryPoint entryPoint) {
        walletImplementaion = new Wallet(entryPoint, address(this));
    }
    ...
    function getAddress(
        ...
        // Encode the proxyContract's constructor arguments which include the address walletImplementation and the walletInint
        bytes memory proxyConstructor = abi.encode(
            address(walletImplementaion),
            walletInit
        );
        // Encode the creation code ERC1967Proxy along with the encoded proxyConstructor data
    ...
    function createAccount(
        ...
        // Check if the code at the couterfactual address is non-empty
        ...
        // If the code is emoty, deploy a new Wallet
        bytes memory walletInit = abi.encodeCall(Wallet.initialize, owners);
