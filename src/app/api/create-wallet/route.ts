...
export async function POST(req: NextRequest) {
    try {
        ...
        // Generate a random salt, convet it to hexadecimal, and prepend "0x"
        ...
        const response = await prisma.wallet.create({
            data: {
                salt: salt,
                signers: signers.map((s: string) => s.toLowerCase()), // Convert all signer addresses to lowercase for consistency
                isDeployed: false,
        ...
        // log any errors and return them as a JSON reponse 
        console.log(e);
