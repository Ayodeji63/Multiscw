...
import { entryPointContract, getWalletContract, provider, walletFactoryContract } from "@/utils/getContracts";
...
export async function getUserOpForETHTransfer(walletAddress:string, owners: string[], salt: string, toAddress: string, value: BigNumber, isDeployed?: boolean) {
    try {
        ...
        const encodedCallData = walletContract.interface.encodeFunctionData("execute", [toAddress, value, initCode]);
